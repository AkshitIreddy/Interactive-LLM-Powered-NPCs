...
| MuseTalk 1.5 | Generic screen-space lip-sync | Code repository license plus transitive model/assets | Public experimental comparator; standalone Windows test functionally passed, but **blocked** for product use | The 2026-08-29 synthetic image + real stock-audio run attested 4.175 GiB of inputs and produced a 1.579 s/39-frame MP4, but took about 102 s end-to-end through the upstream batch path. It fails the live-latency gate; retain outside the pack catalog until a persistent, cancellable, game-contention-qualified runner and full license/security review pass. |
