...
| Audio2Face-3D regression v2.3 → project-owned 2D mouth residual | Generic screen-space lip-sync control signal | SDK and model terms must be reviewed separately | Unqualified architecture candidate; no native game-rig path or pack claim | Pin exact SDK/model revisions and licenses; validate coefficient mapping, Windows runtime, cancellation, mask/occlusion, latency, RAM/VRAM and game impact. |
| NVIDIA Maxine AR SDK LipSync / NGC feature | Generic direct-video lip-sync | Access-controlled NVIDIA SDK/feature terms; a hosted NIM key alone does not prove entitlement | Conditional experiment only | Verify exact artifact/license/access; supported Windows/GPU path; synchronized frame/audio contract; startup/pre-roll; residual isolation; size, VRAM, quality, latency, and game impact. |
| MuseTalk 1.5 | Generic screen-space lip-sync | Code repository license plus transitive model/assets | Offline comparator; standalone Windows batch path functionally passed, but **blocked** for live/product use | The 2026-08-29 synthetic image + real stock-audio run attested 4.175 GiB of inputs and produced a 1.579 s/39-frame MP4, but took about 102 s end-to-end through the upstream batch path. It fails the live-latency gate; retain outside the pack catalog unless a persistent, cancellable, game-contention-qualified runner and full license/security review later justify reconsideration. |
...
Lip-sync reference snapshot: [Audio2Face-3D](https://github.com/NVIDIA/Audio2Face-3D), [NVIDIA AR SDK LipSync](https://docs.nvidia.com/maxine/ar/latest/API/Architecture/using-ar-features.html#lipsync), [MuseTalk](https://github.com/TMElyralab/MuseTalk), [EfficientSync](https://arxiv.org/abs/2608.18832), and [FlashLips](https://arxiv.org/abs/2512.20033), refreshed 2026-08-29. Candidate names and upstream descriptions are research inputs, not product benchmarks, package qualification or availability claims.
