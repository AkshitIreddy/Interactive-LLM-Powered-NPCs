    ...
    try {
        if (argc < 5 || argc > 9) {
            throw std::runtime_error(
                "usage: npc_mouth_worker_headless_realistic_proof <pack-root> <portrait.ppm|source-frames-dir> <audio.wav> <output-dir> [tracking-hz:10|15] [review-mouth-atlas-root|-] [offline-sample-cues.tsv|-] [x,y,width,height]");
        }
        ...
        const std::optional<std::filesystem::path> atlas_root = argc >= 7 && std::string_view(argv[6]) != "-"
        ...
        const auto initial_seed = argc == 9 ? parse_review_seed(argv[8]) :
            (moving_source ? NormalizedRect{0.12, 0.08, 0.76, 0.88} :
        ...
        if (argc >= 8 && std::string_view(argv[7]) != "-") {
        ...
        for (std::size_t sample = 0; sample < warmups + measured_samples; ++sample) {
            ...
            work.seed_face_bounds = initial_seed;
