...
- Added the C++20/WinRT media broker with real HWND-targeted free-threaded WGC, event-driven `IAudioClient3` capture/render, bounded PCM rings, transparent no-activate DirectComposition residual presentation, Desktop Duplication fallback, DPI-aware geometry, device-loss recovery, and fail-open visual gating. Cross-process shared media, audio conversion, and qualified HDR shaders remain integration gates.
...
- Added real hosted adapter implementations for OpenAI, Anthropic, Gemini, Groq/OpenAI-compatible, and Cohere LLMs; Deepgram, AssemblyAI, ElevenLabs, and OpenAI STT; and Cartesia, ElevenLabs, Inworld, and Deepgram TTS, all behind mockable transports with cancellation, privacy, and redaction tests.
- Added explicit NVIDIA NIM routes under one user-owned key: dedicated chat and 2048-dimensional embedding adapters with successful bounded synthetic endpoint smoke evidence; an experimental stock-voice Magpie TTS adapter with successful HTTP audio smoke evidence; and a Nemotron streaming-ASR adapter whose live gRPC audio qualification remains pending. Magpie gRPC streaming is fixture/replay-tested but not live-qualified, and reranking remains non-selectable because the tested hosted endpoints were unavailable. No NVIDIA route is automatically selected, used as a fallback, or represented as production-ready.
...
- Join the exercised WGC/DirectComposition/WASAPI paths to the runtime through cross-process D3D-handle synchronization, OS shared-memory mapping, audio conversion, and qualified HDR shaders.
...
- Live-qualify NVIDIA Nemotron ASR audio and Magpie gRPC streaming before making those routes selectable; keep NVIDIA reranking unavailable until an eligible hosted endpoint is demonstrated.
