...
The current local-runtime decisions are recorded in the [Windows/NVIDIA AI runtime selection](docs/research/windows-nvidia-ai-runtime-selection-2026-08-30.md), the separate [Windows/NVIDIA local LLM runtime selection](docs/research/windows-nvidia-llm-runtime-selection-2026-08-30.md), the measured [real-time talking-face selection](docs/research/realtime-talking-face-selection-2026-09-01.md), the [2026 low-latency overhaul research](docs/research/realtime-lipsync-overhaul-2026-09-04.md), and the current [source-preserving v53 headless proof](docs/research/headless-realistic-lipsync-proof-2026-09-04-v6.md). The procedural v32 result and broad-atlas v37/v5 result are rejected visual evidence, even where their older numeric gates passed. The v53 prototype instead preserves the moving source's lips and exterior face, deforms that current geometry, and transfers only an observed oral interior from a one-time enrollment reference. Its stricter audit passed all recorded geometry and containment gates, but its retained Jason fixture is driven only by RMS aperture and therefore does not prove phoneme-accurate synchronization. The Python inspection renderer measured `54.907 ms` mean / `73.153 ms` p95 and is not the product hot path. Separately, the latest native 1920×1080, 250-iteration CPU benchmark measured `6.402 ms` geometric mean, `6.323 ms` direct-atlas mean, and `4.612 ms` mean / `4.682 ms` p50 / `7.004 ms` p95 / `7.264 ms` p99 for atlas worker select-and-compose, with zero GPU VRAM; all six native CTest suites passed. That performance evidence does not rescue the latest v66 visual port, which is rejected for a dark hole, missing teeth, distorted lips, and excessive upper-lip damage. MuseTalk or image generation may be used only as an optional, one-time enrollment teacher; no heavy model runs per dialogue turn or remains resident during gameplay. Desktop/live-game presentation, generated-reference parity, installer qualification, and a selectable generic visual route remain open. Because GUI testing was intentionally stopped after display-safety concerns, the general local visual route remains disabled and fails open to the untouched frame plus audio/subtitles.
