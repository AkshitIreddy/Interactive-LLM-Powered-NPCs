...
- [ ] Live-qualify NVIDIA Nemotron streaming ASR over gRPC and Magpie streaming gRPC audio before those experimental routes become selectable; keep hosted reranking unavailable until an eligible route passes.
- [ ] Qualify a generic screen-space lip-sync worker independently. The current application has no selectable NVIDIA or other face/lip-sync route, and per-game rig/mod integration is out of scope.
