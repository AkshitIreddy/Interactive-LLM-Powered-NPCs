...
- [x] Qualify the isolated headless screen-space mouth worker against a moving realistic game-idle sequence and real NVIDIA Magpie API WAV. The pinned OpenSeeFace MNV3+LM1 CPU path measured 25.423 ms p95 moving inference at its admitted 15 Hz signal rate and 1.488 ms p95 compositing at 960x720/30 FPS with 0 GPU VRAM, 26 changed adjacent source frames, 42 distinct output frame digests, and 31 frames above the material mouth-motion gate. Representative closed/open/return frames were visually inspected after adding upper-lip preservation, lower-jaw-biased motion, restrained exposure-matched teeth/tongue detail, and causal PCM attack/release smoothing. A CC0 Godot character was correctly rejected below the unchanged landmark-confidence floor. This qualifies the isolated CPU visual core only; the last desktop app attempt did not present lip-sync, so WGC/broker/DirectComposition presentation, selectable-pack activation, unseen-character quality, commercial-game footage, and live-game certification remain open.
