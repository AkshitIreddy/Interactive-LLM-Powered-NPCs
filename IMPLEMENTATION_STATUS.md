...
- [ ] Regenerate and reconcile the local review installer with the replacement source-preserving CPU mouth worker. The prior v9 package passed closed-world reconciliation but its mouth compositor was later rejected visually. The v10 local review directory overlays the replacement warning-as-error-built sidecar and matching manifest without launching the app, but that directory is not newly reconciled installer evidence.
...
- [x] Replace and requalify the rejected headless mouth proof. The first run passed numeric gates but failed visual review with a detached procedural slit, flat teeth strip, and female Aria audio on a male subject. The replacement uses explicit stock male Jason audio, a lip-local source-pixel warp, source-derived cavity texture, and no generated teeth/tongue. It measured 29.251 ms p95 moving OpenSeeFace inference at adaptive 10 Hz and 1.337 ms p95 compositing at 960x720/30 FPS with 0 GPU VRAM, 32 changed adjacent source frames, 40 distinct output digests, and 27 frames above the material mouth-motion gate. This qualifies only the isolated CPU component; WGC/broker/DirectComposition presentation, selectable-pack activation, broader face/camera quality, commercial-game footage, and live-game certification remain open.
