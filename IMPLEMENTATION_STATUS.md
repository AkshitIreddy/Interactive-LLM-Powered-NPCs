...
- [x] Qualify the isolated headless screen-space mouth worker against a realistic portrait and real NVIDIA Magpie API WAV. The pinned OpenSeeFace MNV3+LM1 CPU path measured 30.210 ms p95 inference and 1.657 ms p95 compositing at 960x720/30 FPS with 0 GPU VRAM, 34 changed adjacent frames, 32 distinct frame digests, and 28 frames above the material mouth-motion gate. Representative closed/open/return frames were visually inspected after rejecting and correcting a diagonal-cavity artifact. This qualifies the isolated CPU visual core only; the last desktop app attempt did not present lip-sync, so WGC/broker/DirectComposition presentation, selectable-pack activation, unseen-character quality, and live-game certification remain open.
