...
- [x] Qualify the isolated headless screen-space mouth worker against two license-safe real-person motion clips and a real NVIDIA Magpie API WAV. The primary unobstructed proof measured 28.825 ms p95 moving OpenSeeFace inference at an adaptive 10 Hz signal rate and 2.027 ms p95 compositing at 960x720/30 FPS with 0 GPU VRAM, 34 changed adjacent source frames, 42 distinct output frame digests, and 31 frames above the material mouth-motion gate. Representative closed/open/return frames were visually inspected after adding upper-lip preservation, lower-jaw-biased motion, shallow-cavity softening, restrained exposure-matched teeth/tongue detail, and causal PCM attack/release smoothing. An independent wind/idle clip also passed; an extreme close-up, a darker neutral portrait below the 0.82 landmark floor, and a CC0 Godot character were correctly rejected without weakening product gates. This qualifies the isolated CPU visual core only; the last desktop app attempt did not present lip-sync, so WGC/broker/DirectComposition presentation, selectable-pack activation, broader demographic/camera-scale quality, commercial-game footage, and live-game certification remain open.
