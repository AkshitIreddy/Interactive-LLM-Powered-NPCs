...
The current local-runtime decisions are recorded in the [Windows/NVIDIA AI runtime selection](docs/research/windows-nvidia-ai-runtime-selection-2026-08-30.md), the separate [Windows/NVIDIA local LLM runtime selection](docs/research/windows-nvidia-llm-runtime-selection-2026-08-30.md), the measured [real-time talking-face selection](docs/research/realtime-talking-face-selection-2026-09-01.md), the [rejected first headless render](docs/research/headless-realistic-lipsync-proof-2026-09-03.md), the [rejected upper-lip-hole revision](docs/research/headless-realistic-lipsync-proof-2026-09-03-v2.md), and the [upper-lip-protected replacement](docs/research/headless-realistic-lipsync-proof-2026-09-04-v3.md). Stock MuseTalk is rejected from the resident gameplay path at 8.95 complete FPS and 7,617 MiB before a game. The active lightweight path uses pinned OpenSeeFace CPU landmarks plus an exact-frame procedural mouth residual. The current 960x720 candidate used the explicitly selected stock male NVIDIA Magpie voice Jason and measured 58.284 ms p95 moving tracking at adaptive 10 Hz, 1.847 ms p95 compositing at 30 FPS, 0 GPU VRAM, 32 changed adjacent source frames, 18 frames above its material mouth-motion gate, and zero protected-upper-lip darkening. It keeps the source upper lip stationary and opens the lower jaw without synthesizing teeth or tongue. It remains a causal talking-mouth renderer, not a neural full-head generator. Desktop broker presentation did not succeed in the last live attempt, so no local visual pack is selectable or release-ready.

## Review the local Windows build

The original synthetic Eclipse Harbor target is prepared and hash-verifiable on the maintainer machine. The reconciled v9 package is retained as package evidence but contains the subsequently rejected mouth compositor. A v10 local review directory replaces only that mouth sidecar with the warning-as-error-built, PE-audited source-preserving version and updates its sidecar manifest; it has not been launched or promoted to newly reconciled installer evidence. Machine-local paths and hashes are recorded in `HANDOFF.md`, not published as a release. The build is unsigned and `unchecked-development-package`; the [local 2.0 review guide](docs/product-rework/local-review.md) remains a blocked, non-authoritative placeholder until safe desktop presentation, screenshot/demo evidence, and the remaining RC gates are complete.
