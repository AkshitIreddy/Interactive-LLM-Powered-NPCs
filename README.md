...
The checkout now contains the Response Console and Tauri shell, its supervised authenticated Rust runtime sidecar, versioned protocol/runtime contracts, SQLite memory, hosted LLM/STT/TTS adapter cores, optional visual-worker contracts, provider/model catalogs, a production-capable verified Model Manager core, exercised native WGC/WASAPI/DirectComposition paths, 20 validated profiles, deterministic simulation, and a safe Windows game-load harness. Developer-supplied Cohere, ElevenLabs, AssemblyAI, and NVIDIA NIM credentials have passed bounded authentication or synthetic smoke tests without entering the repository. Ordinary typed turns resolve an immutable saved-loadout snapshot and construct supported selected hosted LLM routes; selected-route TTS constructs ElevenLabs in ordinary builds. NVIDIA hosted trial routes (LLM, embeddings, and Magpie TTS) are available only for private evaluation in the isolated Release review (`io.github.akshitireddy.interactive-npcs.review`) or Debug (`io.github.akshitireddy.interactive-npcs.debug`) application namespace after a current provider-wide NVIDIA API Trial Terms acknowledgement and user credential. Magpie additionally requires an authenticated discovered stock voice; the base public production namespace cannot activate these routes, and promotion/publication remain unsupported. Cartesia, Deepgram, and Inworld TTS remain adapter-core/catalog work and are rejected as executable selections. NVIDIA evidence currently covers chat, 2048-dimensional embeddings, Magpie HTTP voice discovery/synthesis, and an authorized stock-voice Magpie stream through the concrete fixed-origin Riva gRPC transport. Nemotron streaming ASR timed out during bounded live qualification and remains disabled, and hosted reranking was unavailable at the tested routes. Authenticated broker PCM, receipt-backed native subtitle presentation, and exact current-frame source transport now exist, but this is still not a game-ready public release: qualified visual-worker/landmark-pack and HDR presentation, generic screen-space lip-sync qualification, live-game certification, a clean Windows 10/11 VM matrix, release signing, and controlled performance benchmarks remain gated work. No current app or installer is a supported end-user handoff until the final four-sidecar review package and installed-distribution evidence are regenerated for the frozen source. See [CHANGELOG.md](CHANGELOG.md) and [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md).

The current local-runtime decisions are recorded in the [Windows/NVIDIA AI runtime selection](docs/research/windows-nvidia-ai-runtime-selection-2026-08-30.md), the separate [Windows/NVIDIA local LLM runtime selection](docs/research/windows-nvidia-llm-runtime-selection-2026-08-30.md), the measured [real-time talking-face selection](docs/research/realtime-talking-face-selection-2026-09-01.md), and the [headless realistic lip-sync proof](docs/research/headless-realistic-lipsync-proof-2026-09-03.md). Stock MuseTalk is rejected from the resident gameplay path at 8.95 complete FPS and 7,617 MiB before a game. The active lightweight path uses pinned OpenSeeFace CPU landmarks plus an exact-frame procedural mouth residual: the accepted unobstructed real-person 960x720 headless run measured 28.825 ms p95 tracking at an adaptive 10 Hz, 2.027 ms p95 compositing at 30 FPS, 0 GPU VRAM, 34 changed adjacent source frames, and 31 frames above its material mouth-motion gate against a real NVIDIA Magpie WAV. An independent wind/idle clip also passed, while three unsupported face/camera conditions failed closed. It is a causal talking-mouth renderer, not a neural full-head generator. Desktop broker presentation did not succeed in the last live attempt, so no local visual pack is selectable or release-ready.

## Review the local Windows build

The original synthetic Eclipse Harbor target is prepared and hash-verifiable on the maintainer machine. The app and installer paths will be published here only after the current source is frozen, all four required sidecars are staged, and the local-review package plus installed-distribution smoke evidence agree. Until then, an older debug executable or installer output is not the review handoff. The [local 2.0 review guide](docs/product-rework/local-review.md) is deliberately a blocked, non-authoritative placeholder; do not use it to launch anything. The final package gate will regenerate that file with the exact launch order, safe provider test, verified commands, hashes, screenshot/demo evidence, and limitations for one frozen candidate.
