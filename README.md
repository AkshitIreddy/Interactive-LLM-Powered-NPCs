...
The original synthetic Eclipse Harbor target is prepared and hash-verifiable on the maintainer machine. A source-clean local review build now contains the refined four-sidecar set and passes closed-world installed-file reconciliation. Its machine-local paths and hashes are recorded in `HANDOFF.md`, not published as a release. The build is unsigned and `unchecked-development-package`; the [local 2.0 review guide](docs/product-rework/local-review.md) remains a blocked, non-authoritative placeholder until safe desktop presentation, screenshot/demo evidence, and the remaining RC gates are complete.
